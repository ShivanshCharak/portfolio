...
import { Projects, projectsData, Project  } from "./utils/projectdata";
...
export default function ProjectsViewer({
  ...
  const [animateInTransitionDirection,setAnimateInTransitionDirection] = useState<"up" | "down">();
  const [animateIn, setAnimateIn] = useState(false)
  const containerRef = useRef<HTMLDivElement>(null);
  {console.log("index",index,"index",index,projectKeys[index],projects)}
  
  useEffect(() => {
    const body = document.querySelector("body");
    
    if (index >= 0 && body) {  // Combined condition with proper null check
      body.style.overflow = "hidden";
      setIndex(index);
      setCurrentProject(projects[projectKeys[index]]);
    }
    
    return () => {
      const cleanupBody = document.querySelector("body");
      if (cleanupBody) {
        cleanupBody.style.overflow = "";
      }
    };
  }, [index, projects, projectKeys]);  // Added missing dependencies
  ...
  function navigateProject(command: "prev" | "next") {
    ...
    setIsAnimating(true);
    setAnimateIn(true)
    setAnimateInTransitionDirection(command==="prev"?"up":"down")
    ...
    setTimeout(() => {
      ...
      console.log("Current index changing on lcicked",)
  ...
  console.log(index,projects[projectKeys[index]])
        ...
        <div className="w-full flex justify-center py-20 px-20">
          ...
          <section className="w-[300px] flex justify-between">
            <span className="Inter text-[14px]">
              {index > 0
                ? projects[projectKeys[index - 1]].heading
